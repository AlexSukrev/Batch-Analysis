...
1) "1_scmos_batch_analysis":
	...
	# The last part of the code is commented out, as they seem to be used for test purpose only. 

2) "2_master_python_analysis":

	This script will convert all STORM .hdf5 fitting files into .TIF image files. 

	The path to the expfolder should be changed for your experiment. All data files should be stored in a single folder
	entitled "Acquisition". The # of processes to run concurrently (max_processes) should be changed to suit machine 
	usage. 
	
	# A warning related to parameter format is given, but seems doesn't matter. ---7.30.2018

3) "3_bead_image_processing":

	This script will load the FFC .dax files, identify the appropriate frames of interest, stretch the histograms for
	each FFC image to normalize them across the entire set and save out the images. This script also saves the final
	FFC images for use in generating the lens distortion correction. Lastly, the script performs the lens distortion
	correction using a 3x3 array of dense bead field FFC images.  

	The path to the expfolder should be changed for your experiment. All data files should be stored in a single folder
	entitled "Acquisition". The # of processes to run concurrently (max_processes) should be changed to suit machine 
	usage. The frames and image size will need to be changed if acquisition parameters differ from 640 x 640 standard.
	During the lens distortion correction, FIJI will throw a dialogue to adjust the lambda parameter (see lens distortion
	...
	this part of the error correction should either be fixed, or abandoned. 
	
	# Fiji in accound 'Colenso' is used in this script. If FIji doesn't run, open C:/Users/Colenso/ in the file explorer manually to obtain the authority. 
	# Somehow the python script 'C:\\Users\\Colenso\\Fiji.app\\macro\\fiji_discoor_make.py' doesn't work on my account... It can't fetch the argument from 3_bead_image_processing.py. So I went into the script and change the path manually. The old codes are comment out. 

4) "4_XY_alignment":

	This script builds folder structure for aligning images for each physical section and for saving out all processed
	and merged images. The code finds the conventional images from .dax files, stretches the histograms, applies the FFC
	...
	# In fiji_distcorr1field_hcam.py, line 94 is changed to '647Visconv' rather than "647IRconv", since 647IRconv images are completely dark now. 
